		...
		<br><br><br>
			...
			<table class="table table-striped">
				...
				<tbody>
					<c:forEach var="m" items="${totallist}">
						<tr>
							<td><a href="userInfo.co?id=${m.id}&user_no=${m.user_no}">${m.id}</a></td>
							<td>${m.nickname}</td>
							<td>${m.user_grantView}</td>
							<td>${m.tel}</td>
			...
			<div>
				<ul class="pagination justify-content-center">
					...
					<c:if test="${page >1 }">
						<li class="page-item">
							<a href="userList.co?page=${page-1}&search_field=${search_field}&search_word=${search_word}" class="page-link">이전</a>&nbsp;				
						</li>
					</c:if>
